...
![Alt text](./NioEventLoop的启动过程.png)
...
**pipeline.fireChannelRead** 正好就是我们在第二章 **Netty 源码分析之 二 贯穿Netty 的大动脉 ── ChannelPipeline (二)** 中分析的 **inbound** 事件起点。 当调用了 **pipeline.fireIN_EVT()** 后， 那么就产生了一个 **inbound** 事件， 此事件会以 **head -> customContext -> tail** 的方向依次流经 ChannelPipeline 中的各个 handler。
